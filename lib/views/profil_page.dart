...
class _ProfilPageState extends State<ProfilPage> {
  ...
  void initState() {
    super.initState();
    _fetchUserData();
   // _confirmPassword = false; // Initialisation de _confirmPassword à false
  }

  // Pour libérer les ressources lorsque le controleur n'est plus utilisé
  @override
  void dispose() {
    _passwordController.dispose();
    super.dispose();
  ...
  Future<void> _fetchUserData() async {
    try {
      print('Fetching user data in ProfilPage...');
      await _controller.fetchUser();
    } catch (e) {
      setState(() {
        _errorMessage =
  ...
  Future<void> _uploadImage() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        // Récupérez l'URL de téléchargement après le succès du téléchargement
        final photoUrl = await taskSnapshot.ref.getDownloadURL();

        // Mettez à jour l'URL de la photo de l'utilisateur
        await _controller.updateUserProfile({'photoUrl': photoUrl});
        await user.updatePhotoURL(photoUrl);

        // Actualisez les données de l'utilisateur
        await _fetchUserData();
      }
    } catch (e) {
              ...
              content: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    onChanged: (value) {
                      setState(() {
                        _confirmPassword = value; // Comparer les mots de pass;
                      });
                    },
                    decoration: InputDecoration(
                        hintText: 'Confirmez votre nouveau mot de passe'),
              ...
              actions: <Widget>[
                ...
                TextButton(
                  onPressed: () {
                    bool passwordsMatch = _newPassword != null && _confirmPassword != null && _newPassword == _confirmPassword;
                    Navigator.of(context).pop(passwordsMatch);
                    //Navigator.of(context).pop(_confirmPassword = true );
                  },
                  child: Text('Confirmer'),
                ),
              ],
  ...
  Future<void> _confirmLogout() async {
    ...
    print("Confirm: $confirm"); // Vérifier la valeur de confirm

    if (confirm != null && confirm) {
     await _logout(); //Pour attendre la déconnexion
    }else {
      print("Déconnexion annulée"); // Pour confirmer que la déconnexion est annulée
    }
  }
  ...
  Future<void> _logout() async {
    ...
    await FirebaseAuth.instance.signOut();
    Navigator.of(context).pushReplacementNamed('/login'); // Rediriger vers la page de connexion
  }

  @override
  Widget build(BuildContext context) {
    print('Building ProfilPage...');
    if (_controller.isLoading) {
      return Center(child: CircularProgressIndicator());
    }
